...
function imgIConBuilder(name, hnd, type , title) {
  
    hnd = "'" + hnd + "'";
    title = title.replace(' ',
                        '%20'); // in this line if title has space replace it with %20 to prevent unwanted character reference 
    title = title.replace(' ',
                           '');
    var onclick = " ondblclick=DoWork('" + title +"'," + hnd + "," + type + ")";
    var img = '<img class="imgIcon"  ' + onclick + '  src="' + name + '"></img>';
    return (img);
}
...
function generateWindow(URL , templateWindow , layoutList , callerType , applicationName)
{
    ...
    var title = pwindow.find(".formControl > span ");//.find("iframe:eq(0)");
    pwindow.click(function () {
        ...
        $(".formControl").css("background-color",
                           "white");
        pwindow.find(".formControl:eq(0)").css("background-color","#4169E1");
        //setFocus(templateWindow);
       
    });

    pwindow.focusout(function () {
        $(".formControl").css("background-color",
                          "white");
    });
...
function setFocus(focusedWindow)
{
    $(".pcontainer").css("background-color",
                            "white");
    var pContainer = $(focusedWindow).find(".pcontainer:eq(0)");
    if (pContainer == null)
        console.log('no pcontainer');
    pContainer.css("background-color",
                            "red");
}